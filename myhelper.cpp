...
//设置全局为plastique样式
void myHelper::SetPlastiqueStyle()
{
    QApplication::setStyle(QStyleFactory::create("Plastique"));
    QApplication::setPalette(
        QApplication::style()->standardPalette());//替换应用程序的颜色组合
}
...
//设置编码为系统字体编码
void myHelper::SetSystemCode()
{
    QTextCodec*pCodec =
        QTextCodec::codecForName("System"); //获取系统字体编码
...
//16进制字符串转10进制
uchar myHelper::StrHexToDecimal(QString strHex)
{
    bool ok;
...
//10进制字符串转10进制
uchar myHelper::StrDecimalToDecimal(QString strDecimal)
{
    bool ok;
...
//播放声音
void myHelper::PlaySound(QString soundName)
{
    QSound::play(soundName);
}

//获取当前日期时间星期
QString myHelper::GetLongDateTime()
{
...
//打开网址
void myHelper::OpenUrlOrExe(QString url)
{
    QDesktopServices::openUrl(QUrl(url));
}

//判断是否闰年
bool myHelper::IsLeapYear(QString strDate)
{
...
//窗体居中显示
void FormHelper::FormInCenter(QWidget *frm)
{
...
//窗体没有最大化按钮
void FormHelper::FormNoMaxButton(QWidget *frm)
{
    frm->setWindowFlags(Qt::WindowMinimizeButtonHint);
}

//窗体没有最大化和最小化按钮
void FormHelper::FormOnlyCloseButton(QWidget *frm)
{
    frm->setWindowFlags(Qt::WindowMinMaxButtonsHint);
    frm->setWindowFlags(Qt::WindowSystemMenuHint);
}

//设置窗体透明度
void FormHelper::FormOpacity(QWidget *frm, double value)
{
    frm->setWindowOpacity(value);
}

//窗体不能改变大小
void FormHelper::FormNotResize(QWidget *frm)
{
    frm->setFixedSize(frm->width(), frm->height());
}

//窗体前端显示
void FormHelper::FormOnTop(QWidget *frm)
{
    frm->setWindowFlags(Qt::WindowStaysOnTopHint);
}

//窗体后端显示
void FormHelper::FormOnBottom(QWidget *frm)
{
    //frm->setWindowFlags(Qt::WindowStaysOnBottomHint);
}

//获取桌面大小
QSize FormHelper::GetDesktopSize()
{
    QDesktopWidget desktop;
    return QSize(desktop.availableGeometry().width(),
                 desktop.availableGeometry().height());
}

//设置指定窗体为plastique样式
void FormHelper::SetPlastiqueStyle(QWidget *frm)
{
    frm->setStyle(QStyleFactory::create("Plastique"));
}

void FormHelper::SetCustomStyle(QWidget *frm)
{
...
//改变样式
bool FormHelper::InstallStyle(QWidget* window, QString styleFile)
{
...
//获取选择文件
QString FileHelper::GetFileName(QString filter)
{
    return QFileDialog::getOpenFileName(NULL, tr("选择文件"),
                                        QCoreApplication::applicationDirPath(), filter);
}

//获取选择文件集合
QStringList FileHelper::GetFileNames(QString filter)
{
    return QFileDialog::getOpenFileNames(NULL, tr("选择文件"),
                                         QCoreApplication::applicationDirPath(), filter);
}

//获取选择文件夹中的文件
QStringList FileHelper::GetFolderFileNames(QStringList filter)
{
...
//文件是否存在
bool FileHelper::FileExists(QString strFile)
{
...
//文件夹是否存在
bool FileHelper::FolderExists(QString strFolder)
{
...
//移除文件
bool FileHelper::RemoveFile(QString strFile)
{
    return QFile::remove(strFile);
}

//重命名文件
bool FileHelper::RenameFile(QString oldName, QString newName)
{
    return QFile::rename(oldName, newName);
}

//获取文件目录
QString FileHelper::GetBasePath(QString strFullPath)
{
...
//获取拓展名
QString FileHelper::GetExtension(QString strFile)
{
...
//获取文件夹名称
QString FileHelper::GetFolderName(QString strFullPath)
{
...
//复制文件
bool FileHelper::CopyFile(QString sourceFile, QString targetFile)
{
    if (FileExists(targetFile))
    {
        int ret = QMessageBox::information(NULL, tr("提示"),
                                           tr("文件已经存在，是否替换？"), QMessageBox::Yes | QMessageBox::No);
...
//判断是否是文件
bool FileHelper::IsFile(QString strFile)
{
    QFileInfo fileInfo(strFile);
...
//判断是否是文件夹
bool FileHelper::IsFolder(QString strFolder)
{
...
//追加文本
bool FileHelper::AppendText(QString strFile, QString strContent )
{
...
//创建文件夹
bool FileHelper::CreateFolder(QString strFolder)
{
    QDir dir;
...
//创建文件
bool FileHelper::CreateFile(QString strFile)
{
    QFile file(strFile);
    if (!file.open(QIODevice::WriteOnly))
    {
...
//读取文件内容
QString FileHelper::GetFileContent(QString strFile)
{
    QFile file(strFile);
    QString txt;
    if (!file.open(QIODevice::WriteOnly))
